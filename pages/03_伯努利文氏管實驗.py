...
if page == "實驗目的":
    st.markdown("""
    ## 一、實驗目的
    流體力學中，伯努利方程式描述流體沿著一條穩定、非粘滯性及不可壓縮 的流線移動，其壓力、速度及高度的變化形成一關係式，此一關係式對於流體力 學中許多運動的特性做了合理的解釋，例如：棒球的運動軌跡，飛機機翼的昇力 等。而本裝置的目的為幫助致力於流體力學學習的學員，藉由文氏管的壓力與速 度的量測，來檢驗伯努利方程式能量守恆與質量守恆的概念，如此對流體運動中 速度與壓力的關係有較深刻的認識與了解。另外，本裝置所使用的標準風量產生 裝置，係根據國際規範AMCA 210-16製造而得，其精確度的可在3 % 以內，其 結構及流量計算原理與目前廣泛應用於工業界風扇及系統阻抗測試之AMCA風 洞是一致的，而學員透過實際量測，可對風洞流量量測的原理與方法做深入的了 解。 
