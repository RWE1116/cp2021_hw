...
if page == "實驗目的":
    ...
    try:
        with open(r"Y:/a/downloads/第5組 實驗5.溫度與散熱實驗.pdf", "rb") as file:
            st.download_button(
                label="下載 第5組 實驗5.溫度與散熱實驗 PDF 檔案",
                data=file.read(),
                file_name="第5組 實驗5.溫度與散熱實驗.pdf",
                mime="application/pdf"  # 設置 MIME 類型為 PDF
            )
    except FileNotFoundError:
        st.error("找不到文件：Y:/a/downloads/第5組 實驗5.溫度與散熱實驗.pdf")

    # 下載 第5組 實驗5.溫度與散熱實驗 DOC 檔案
    try:
        with open(r"Y:/a/downloads/第5組 實驗5.溫度與散熱實驗.doc", "rb") as file:
            st.download_button(
                label="下載 第5組 實驗5.溫度與散熱實驗 DOC 檔案",
                data=file.read(),
                file_name="第5組 實驗5.溫度與散熱實驗.doc",
                mime="application/msword"  # DOC 文件的 MIME 類型
            )
    except FileNotFoundError:
        st.error("找不到文件：Y:/a/downloads/第5組 實驗5.溫度與散熱實驗.doc")
